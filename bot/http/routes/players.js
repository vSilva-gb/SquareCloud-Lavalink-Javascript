	...
	router.get('/:guildId/add', async (req, res) => {
		...
		if (player) {
			const guild = client.guilds.cache.get(req.params.guildId),
				data = await player.search(req.query.song, client.users.cache.get(req.query.user));
	...
	router.get('/:guildId/remove', (req, res) => {
		...
		if (player) {
			...
			if (!endNum) {
				...
				client.channels.cache.get(player.textChannel)?.send(guild.translate('music/remove:REMOVED', { TITLE: title }));
			} else {
				...
				return client.channels.cache.get(player.textChannel)?.send(guild.translate('music/remove:REMOVED_MULTI', { NUM: songsToRemove + 1 }));
			}
		} else {
			res.status(400).json({ error: 'No music playing in that guild.' });
		}
