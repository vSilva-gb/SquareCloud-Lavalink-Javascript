...
module.exports = Object.defineProperties(TextChannel.prototype, {
	...
	error: {
		value: function(key, args, returnValue) {
			try {
				const emoji = this.permissionsFor(this.client.user).has('USE_EXTERNAL_EMOJIS') ? this.client.customEmojis['galcross'] : ':negative_squared_cross_mark:';
	...
	success: {
		value: function(key, args, returnValue) {
			try {
				const emoji = this.permissionsFor(this.client.user).has('USE_EXTERNAL_EMOJIS') ? this.client.customEmojis['galcheck'] : ':white_check_mark:';
				const embed = new MessageEmbed()
					.setColor("#2ECC71")
					.setDescription(`${emoji} **|** ${this.client.translate(key, args, this.guild.settings.Language) ?? key}`);
				if (returnValue) {
